...
def validate_item(path: Path, categories):
    ...
    for forbidden in ["icon", "banner", "screenshots", "supported_modes", "mode_notes"]:
        require(forbidden not in item, f"{path}: {forbidden} is not supported in the Hub repo. Mode availability is handled inside MiSTer Companion.")
    badges = item.get("badges", [])
    require(isinstance(badges, list), f"{path}: badges must be a list when present")
    for badge in badges:
        require(badge not in {"Online Mode", "Offline Mode"}, f"{path}: online/offline support must not be listed as a badge")
    ...
    if item["type"] == "wallpaper_pack":
        source = item.get("wallpaper_source")
        require(isinstance(source, dict), f"{path}: wallpaper_pack items require wallpaper_source")
        require(source.get("type") == "external_database_zip", f"{path}: wallpaper_source.type must be external_database_zip")
        require(isinstance(source.get("database_url"), str) and source["database_url"].strip(), f"{path}: wallpaper_source.database_url is required")
        require(isinstance(source.get("raw_base_url"), str) and source["raw_base_url"].strip(), f"{path}: wallpaper_source.raw_base_url is required")
        require(isinstance(source.get("format"), str) and source["format"].strip(), f"{path}: wallpaper_source.format is required")
    if item["type"] == "rom":
        for key in ["system", "genres", "default_install_path", "allow_custom_install_path", "download"]:
        ...
        if "allowed_extensions" in download or "archive" in download:
            raise ValueError(f"{path}: use download.type and download.install_extensions instead of archive/allowed_extensions")
    return item
...
def main():
    ...
    for cat in categories_list:
        default_thumb = cat.get("default_thumbnail")
        if not default_thumb:
            errors.append(f"categories.json: category {cat.get('id')!r} missing default_thumbnail")
        elif not (ROOT / default_thumb).exists():
            errors.append(f"categories.json: default_thumbnail does not exist: {default_thumb}")
        elif Path(default_thumb).suffix.lower() not in IMAGE_EXTENSIONS:
            errors.append(f"categories.json: default_thumbnail must be PNG, JPG, JPEG, or WEBP: {default_thumb}")
